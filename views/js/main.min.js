...
var adjectives="dark color whimsical shiny noise apocalyptic insulting praise scientific".split(" "),nouns="animals everyday fantasy gross horror jewelry places scifi".split(" ");function generator(a,b){var d=getAdj(a),e=getNoun(b),c=parseInt(Math.random()*d.length),f=parseInt(Math.random()*e.length);return"The "+d[c].capitalize()+" "+e[f].capitalize()}
...
function(){return pizzaIngredients.crusts[Math.floor(Math.random()*pizzaIngredients.crusts.length)]},ingredientItemizer=function(a){return"<li>"+a+"</li>"},makeRandomPizza=function(){for(var a="",b=Math.floor(4*Math.random()),d=Math.floor(3*Math.random()),e=Math.floor(2*Math.random()),c=0;c<b;c++)a+=ingredientItemizer(selectRandomMeat());for(c=0;c<d;c++)a+=ingredientItemizer(selectRandomNonMeat());for(c=0;c<e;c++)a+=ingredientItemizer(selectRandomCheese());a+=ingredientItemizer(selectRandomSauce());
return a+=ingredientItemizer(selectRandomCrust())},pizzaElementGenerator=function(a){var b,d,e,c;b=document.createElement("div");d=document.createElement("div");e=document.createElement("img");c=document.createElement("div");b.classList.add("randomPizzaContainer");b.style.width="33.33%";b.style.height="325px";b.id="pizza"+a;d.classList.add("col-md-6");e.src="images/pizza.png";e.classList.add("img-responsive");d.appendChild(e);b.appendChild(d);c.classList.add("col-md-6");a=document.createElement("h4");
a.innerHTML=randomName();c.appendChild(a);a=document.createElement("ul");a.innerHTML=makeRandomPizza();c.appendChild(a);b.appendChild(c);return b},resizePizzas=function(a){window.performance.mark("mark_start_resize");document.querySelector("#pizzaSize");window.performance.mark("mark_end_resize");window.performance.measure("measure_pizza_resize","mark_start_resize","mark_end_resize");a=window.performance.getEntriesByName("measure_pizza_resize");console.log("Time to resize pizzas: "+a[0].duration+"ms")};
window.performance.mark("mark_start_generating");for(var i=2;100>i;i++){var pizzasDiv=document.getElementById("randomPizzas");pizzasDiv.appendChild(pizzaElementGenerator(i))}function changePizzaSizes(a){for(var b=0,d=document.querySelectorAll(".randomPizzaContainer"),e=d.length;b<e;b++){var c=determineDx(d[b],a);d[b].style.transform="scaleX("+(d[b].offsetWidth+c+"px")+"px)"}}
function determineDx(a,b){var d=a.offsetWidth,e=document.querySelector("#randomPizzas").offsetWidth,c;a:{switch(b){case "1":c=.25;break a;case "2":c=.3333;break a;case "3":c=.5;break a}c=void 0}return(c-d/e)*e}window.performance.mark("mark_end_generating");window.performance.measure("measure_pizza_generation","mark_start_generating","mark_end_generating");var timeToGenerate=window.performance.getEntriesByName("measure_pizza_generation");
console.log("Time to generate pizzas on load: "+timeToGenerate[0].duration+"ms");var frame=0;function logAverageFrame(a){for(var b=a.length,d=0,e=b-1;e>b-11;e--)d+=a[e].duration;console.log("Average time to generate last 10 frames: "+d/10+"ms")}
function updatePositions1(){frame++;var a,b=0;window.performance.mark("mark_start_frame");for(var d=document.querySelectorAll(".mover"),e=d.length;b<e;b++)a=Math.sin(scrollTopPosition/1250+b%5),d[b].style.transform="translateX("+100*a+"px)";window.performance.mark("mark_end_frame");window.performance.measure("measure_frame_duration","mark_start_frame","mark_end_frame");0===frame%10&&(a=window.performance.getEntriesByName("measure_frame_duration"),logAverageFrame(a))}
window.addEventListener("scroll",updatePositions2);function updatePositions2(){scrollTopPosition=body.scrollTop;scrollLeftPosition=body.scrollLeft;frame++;window.performance.mark("mark_start_frame");window.performance.mark("mark_end_frame");window.performance.measure("measure_frame_duration","mark_start_frame","mark_end_frame");if(0===frame%10){var a=window.performance.getEntriesByName("measure_frame_duration");logAverageFrame(a)}}
(function drawFrame(){window.requestAnimationFrame(drawFrame);updatePositions1();changePizzaSizes(size)})();document.addEventListener("DOMContentLoaded",function(){for(var a=0;200>a;a++){var b=document.createElement("img");b.className="mover";b.src="images/pizza.png";b.style.height="100px";b.style.width="73.333px";b.style.left=a%8*256+"px";b.style.top=256*Math.floor(a/8)+"px";document.querySelector("#movingPizzas1").appendChild(b)}updatePositions1()});
function logCRP(){var a=window.performance.timing,b=a.domContentLoadedEventStart-a.domLoading,a=a.domComplete-a.domLoading;document.getElementById("crp-stats").textContent="DCL: "+b+"ms, onload: "+a+"ms"}window.addEventListener("load",function(a){logCRP()});window.requestAnimationFrame||(window.requestAnimationFrame=window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame||window.msRequestAnimationFrame||window.oRequestAnimationFrame||function(a){return window.setTimeout(a,25)});
